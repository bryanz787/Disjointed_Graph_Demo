...
export default function DisjointNodeGraph() {
  ...
  const [selectedAssignments, setSelectedAssignments] = useState(new Set(["all"]));
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);

  // Configuration variables
  const BASE_NODE_RADIUS = 5;
  const INTERACTION_SCALE_FACTOR = 1; // 20% increase per interaction
  ...
  const assignmentMap = new Map(assignments.map(a => [a.assignmentId, a.title]));
  ...
  const handleAssignmentChange = (assignmentId) => {
    ...
    if (assignmentId === "all") {
      if (newSelected.has("all")) {
        ...
        assignments.forEach(assignment => newSelected.add(assignment.assignmentId.toString()));
      } else {
  ...
  const filteredLinks = useMemo(() => {
    ...
    return links.filter(link => selectedAssignments.has(link.assignmentId.toString()));
  ...
  useEffect(() => {
    ...
    const simulation = d3.forceSimulation(simUsers)
      ...
      .force("charge", d3.forceManyBody().strength(-100)) // Reduced from default (-30)
      .force("x", d3.forceX().strength(0.1)) // Reduced from default (0.1)
      .force("y", d3.forceY().strength(0.1)); // Reduced from default (0.1)
    ...
    const node = svg.append("g")
      ...
      .data(simUsers)
      .join("circle")
      .attr("r", d => BASE_NODE_RADIUS + (userInteractionCounts.get(d.id) || 0) * INTERACTION_SCALE_FACTOR) // Scale node size by interaction count
      .attr("fill", d => color(d.group))
      .on("mouseover", function(event, d) {
        tooltip
          .style("display", "block")
          .html(`<strong>ID:</strong> ${d.id}<br/><strong>Name:</strong> ${d.name}<br/><strong>Group:</strong> ${d.group}<br/><strong>Interactions:</strong> ${userInteractionCounts.get(d.id) || 0}`);
      })
    ...
    link
      .on("mouseover", function(event, d) {
        const assignmentTitle = assignmentMap.get(d.assignmentId) || `Assignment ${d.assignmentId}`;
        tooltip
          .style("display", "block")
          .html(`<strong>Users:</strong> ${d.user1} ↔ ${d.user2}<br/><strong>Assignment:</strong> ${assignmentTitle}<br/><strong>Content:</strong> ${d.value}`);
      })
      .on("mousemove", function(event) {
        tooltip
          .style("left", (event.pageX + 16) + "px")
              ...
              {isDropdownOpen && (
                <div style={{
                  ...
                  <label style={{
                    ...
                    backgroundColor: selectedAssignments.has("all") ? '#f0f8ff' : '#fff',
                    fontWeight: selectedAssignments.has("all") ? 'bold' : 'normal'
                  }}>
                    <input
                      type="checkbox"
                  ...
                  {assignments.map(assignment => {
                    const assignmentInteractionCount = links.filter(link => 
                      link.assignmentId === assignment.assignmentId
                    ).length;
                    const isSelected = selectedAssignments.has(assignment.assignmentId.toString());
                    
                    return (
                      <label key={assignment.assignmentId} style={{
                        display: 'flex',
                        alignItems: 'center',
                        gap: '8px',
                        cursor: 'pointer',
                        padding: '10px 15px',
                        borderBottom: assignment.assignmentId === assignments.length ? 'none' : '1px solid #eee',
                        ...
                        <input
                          type="checkbox"
                          checked={isSelected}
                          onChange={() => handleAssignmentChange(assignment.assignmentId)}
                          style={{ margin: 0 }}
                        />
                        {assignment.title} ({assignmentInteractionCount} interactions)
                      </label>
                    );
                  })}
                </div>
